...
function findElementGreaterThan(arr, num) {
  ...
  return "Число, яке задовільняє умову, не знайдено";
}
}
...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((object) => object.name === name);
...
function getArrayKeys(arr) {
  ...
  const arrayKeys = Array.from(Array(arr.length).keys());
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = Array.from(arr.values()).filter((value) => value > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenedArray.map((element) => element ** 2);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastElements.reduceRight((acc, curr) => acc + curr, 0);
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((sum, product) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = reversedArr.findIndex((num) => num < 0);
  if (lastIndex === -1) {
    return -1;
  }
